...
from Xlearn_backend import settings
...
class StaticFileView(BaseView):
    ...
    def do_dispatch(self, *args, **kwargs):
        ...
        content = self.get_file(os.path.join(settings.STATIC_VIEWROOT, rpath))
        ...
        content = self.get_file(os.path.join(settings.STATIC_VIEWROOT, rpath + '/index.html'))
        if content is not None:
