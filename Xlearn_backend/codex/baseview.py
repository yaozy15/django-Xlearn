...
from codex.baseerror import BaseError, InputError, LoginRequired, AuthorityError
...
class APIView(BaseView):
    ...
    def get_account(self, is_manager):
        user = auth.get_user(self.request)
        if user.is_anonymous():
            raise LoginRequired("Login Required")
        if user.extension.isManager != is_manager:
            raise AuthorityError("用户权限错误")
        if user.extension.isManager:
            return user.extension.manager
        else:
            return user.extension.staff
